...
export default function FriendsScreen(props) {
  ...
  function sendConnectionRequest(p) {
    ...
    sb.rpc('request_anon_connection', { p_recipient: p.user_id }).then(function(r){
      ...
      else { hapticMedium(); toastInfo('Friend request sent ✓'); }
      setViewProfile(null);
  ...
  return React.createElement(motion.div, {
    ...
    style:{display:'flex',flexDirection:'column',height:'100%',background:'var(--bg)',overflowY:'auto'}
  },
    /* Header */
    ...
    React.createElement('div', {
      ...
      React.createElement(FilterPill, {
        ...
        onClick: function(){ hapticTap(); setPicker('lang'); }
      }),
      React.createElement(FilterPill, {
        icon:'📍', label:'City',
        value: fCity || null,
        onClick: function(){ hapticTap(); setPicker('city'); }
      }),
      React.createElement(FilterPill, {
        icon:'🏠', label:'From',
        value: fHome || null,
        onClick: function(){ hapticTap(); setPicker('home'); }
      }),
      React.createElement(FilterPill, {
        icon:'💼', label:'Job',
        value: fOcc || null,
        onClick: function(){ hapticTap(); setPicker('occ'); }
      }),
      React.createElement(FilterPill, {
        icon:'👤', label:'Gender',
        value: fGender ? (fGender === 'f' ? 'Girl' : fGender === 'm' ? 'Boy' : 'Other') : null,
        onClick: function(){ hapticTap(); setPicker('gender'); }
      }),
      React.createElement(FilterPill, {
        icon:'🏷', label:'Interests',
        value: fInterests.length > 0 ? (fInterests.length + ' picked') : null,
        onClick: function(){ hapticTap(); setPicker('interests'); }
      }),
      React.createElement(FilterPill, {
        icon:'🟢', label:'Online',
        value: fOnline ? 'Yes' : null,
        onClick: function(){ hapticTap(); setFOnline(!fOnline); }
      }),
    ...
    /* Task A: skeleton strip for Suggested during initial load */
    (loading && suggested.length === 0 && search.length === 0 && activeCount === 0)
      ? React.createElement('div', {style:{flexShrink:0}},
          React.createElement('div', {style:{padding:'6px 18px 10px',display:'flex',alignItems:'center'}},
            React.createElement(Skeleton, {width:160, height:14, radius:8})
          ),
          React.createElement('div', {
            className:'ringin-hscroll',
            style:{display:'flex',gap:12,padding:'0 16px 20px',overflowX:'auto',overflowY:'hidden',flexWrap:'nowrap',flexShrink:0}
          },
            [1,2,3,4,5].map(function(i){
              return React.createElement('div', {
                ...
                React.createElement(Skeleton, {width:'80%', height:14, style:{marginTop:4}}),
                React.createElement(Skeleton, {width:'60%', height:10}),
    ...
    /* Task A: shimmer skeleton during initial load (results not yet arrived) */
    (loading && results.length === 0)
      ? React.createElement('div', {
          className: 'ringin-hscroll',
          style:{display:'flex',gap:12,overflowX:'auto',padding:12,flexWrap:'nowrap',flexShrink:0}
        },
          [1,2,3,4,5].map(function(i){
            return React.createElement('div', {
              key:'sk'+i,
