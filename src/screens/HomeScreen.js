...
export function UserProfileView(props){
  ...
  // Delete-confirm sheet state for UserProfileView (replaces window.confirm).
  ...
  function reallyDeletePostU(postId){
    var snapU2=userPosts.slice();
    setUserPosts(function(prev){return prev.filter(function(x){return x.id!==postId;});});
    sbHome.from('posts').delete().eq('id',postId).then(function(r){
      if(r.error){
        console.error('RingIn Error [deletePostU]:', r.error);
        setUserPosts(snapU2);
  ...
  return React.createElement('div',{style:{display:'flex',flexDirection:'column',height:'100%',background:'var(--bg)',overflowY:'auto',position:'relative'}},
    ...
    postMenuU ? React.createElement('div',{
      ...
      React.createElement('div',{onClick:function(e){e.stopPropagation();},style:{position:'fixed',top:'50%',left:'50%',transform:'translate(-50%,-50%)',background:'rgba(28,24,40,0.45)',backdropFilter:'blur(48px)',WebkitBackdropFilter:'blur(48px)',border:'1px solid rgba(255,255,255,0.1)',borderRadius:'20px',width:'280px',boxShadow:'0 8px 40px rgba(0,0,0,0.35)',overflow:'hidden'}},
        (function(){
          ...
          var items=isOwn?[
            {icon:'🗑️',label:'Delete Post',red:true,fn:function(){setPostMenuU(null);setDelConfU({postId:p.id});}},
    ...
    // Delete-post confirm sheet (replaces window.confirm in UserProfileView).
    React.createElement(ConfirmSheet, {
      ...
      cancelLabel: 'Cancel',
      destructive: true,
      onConfirm: function(){ if(delConfU) reallyDeletePostU(delConfU.postId); setDelConfU(null); },
...
export default function HomeScreen(props){
  ...
  // Delete-confirm sheet state (replaces window.confirm for delete post).
  var delConfS=useState(null); var delConf=delConfS[0]; var setDelConf=delConfS[1];
  // Moderation-confirm sheet state (replaces window.confirm for borderline posts).
  ...
  function reallyDeletePost(postId){
    ...
    setPosts(function(prev){return prev.filter(function(x){return x.id!==postId;});});
    sbHome.from('posts').delete().eq('id',postId).then(function(r){
      if(r.error){
        console.error('RingIn Error [deletePost]:', r.error);
        setPosts(snapBefore);
        toastError('Failed to delete post.');
      }
    });
  ...
  function submitPost(){
    ...
    moderationPromise.then(function(detection){
      ...
      if (detection && detection.action === 'review') {
        // Capture a proceed callback and show ConfirmSheet instead of window.confirm.
        // The callback closes over all the local variables needed to continue posting.
        var proceedWithPost = function(){
          runPostPipeline(detection);
        };
        setModConf({ flags: detection.flags, proceed: proceedWithPost });
        setPosting(false);
        return;
      }
    ...
    function runPostPipeline(detection){
      var session = props.session;
  ...
  return React.createElement(motion.div, {
    ...
    postMenu ? React.createElement('div',{
      ...
      React.createElement('div',{onClick:function(e){e.stopPropagation();},style:{position:'fixed',top:'50%',left:'50%',transform:'translate(-50%,-50%)',background:'rgba(28,24,40,0.45)',backdropFilter:'blur(48px)',WebkitBackdropFilter:'blur(48px)',border:'1px solid rgba(255,255,255,0.1)',borderRadius:'20px',width:'280px',boxShadow:'0 8px 40px rgba(0,0,0,0.35)',overflow:'hidden'}},
        (function(){
          ...
          var isOwn=p.userId===currentUserId;
          var items=isOwn?[
            {icon:'🗑️',label:'Delete Post',red:true,fn:function(){setPostMenu(null);setDelConf({postId:p.id});}},
      ...
      return feedArr.map(function(p){
        ...
        return React.createElement('div', {key:p.id, className:'fpost'},
          ...
          React.createElement('div', {className:'pacts'},
            ...
            React.createElement(motion.button,{className:'pa'+(p.liked?' liked':''),onClick:function(){toggleLike(p.id);},style:{background:'none',border:'none',cursor:'pointer',display:'flex',alignItems:'center',gap:'5px',padding:'8px 2px',fontSize:'13px',fontWeight:p.liked?700:400},whileTap:{scale:1.35},transition:{type:'spring',stiffness:300,damping:18}},
    ...
    // Delete-post confirm sheet (replaces window.confirm for delete post in feed).
    React.createElement(ConfirmSheet, {
      open: !!delConf,
      title: 'Delete post?',
      message: "This can't be undone.",
      ...
      cancelLabel: 'Cancel',
      destructive: true,
      onConfirm: function(){ if(delConf) reallyDeletePost(delConf.postId); setDelConf(null); },
      onCancel: function(){ setDelConf(null); }
    }),
    // Moderation-confirm sheet (replaces window.confirm for borderline content).
    React.createElement(ConfirmSheet, {
      open: !!modConf,
      title: 'Review your post',
      message: modConf ? ('Your post may contain ' + (modConf.flags||[]).join(', ') + '. Post anyway?') : '',
      confirmLabel: 'Post anyway',
      cancelLabel: 'Cancel',
      destructive: false,
      // Supervisor: clear proceed on the captured modConf object so a rapid
      // double-tap from the same React closure can't fire runPostPipeline
      // twice (which would insert two posts — runPostPipeline's own
      // setPosting(true) doesn't guard re-entry within the same synchronous
      // tick because the second call still sees posting=false from React's
      // batched state).
      onConfirm: function(){
        if(!modConf || !modConf.proceed) { setModConf(null); return; }
        var fn = modConf.proceed;
        modConf.proceed = null; // mutate so second click in same tick is a no-op
        setModConf(null);
        try { fn(); } catch(e) { console.warn('[ringin] modConf.proceed threw:', e); }
      },
      onCancel: function(){ setModConf(null); }
    })
  );
}
